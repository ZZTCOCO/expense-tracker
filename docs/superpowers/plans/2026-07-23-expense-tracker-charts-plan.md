...
## 附加：暗黑霓虹 UI + 明暗切换（应需求追加）

- ✅ 全站暗黑霓虹风：Element Plus dark 主题 + 自定义霓虹色板（青/紫/绿/红）+ 深色角落光晕背景 + 玻璃卡片 + 发光数字；图表暗色主题适配
- ✅ 明暗切换：`useTheme` composable（单例 + localStorage 持久化 + 跟随系统）+ 顶部 el-switch（日/月图标）+ index.html 首屏预置防闪烁；浅色模式用偏深霓虹保证对比、关闭辉光；图表随主题切换重算配色
- 验证：计算样式确认渐变应用名/辉光生效；明暗双向切换、持久化、刷新保持、双模式图表渲染均实测通过；浅色可读性视觉确认 OK
